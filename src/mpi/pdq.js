...
const results = {
    ...
    //John Watson: single result, missing NHS Number and GP, duplicate hospital numbers
    ...
    //Martha Hudson: multiple results, one with a date of death recorded
...
function validate (soapMessage) {
  ...
  var soapOk = soapMessage.indexOf(soapEnvelope);
  ...
  var hl7Ok = soapMessage.indexOf(hl7);
  var qpdSegment = soapMessage.indexOf("QPD");
...
function InvokePatientDemographicsQuery(soapMessage){

  try {
      
    const {valid, validationError} = validate(soapMessage);
    if (!valid) {
      return validationError;
    }

    var doc = new xmlParser().parseFromString(soapMessage, 'text/xml');
...
module.exports = { InvokePatientDemographicsQuery }